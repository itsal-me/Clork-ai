...
import { Send, Settings, LogOut, Code2, BrainIcon, Sparkles, Loader2, Copy, Check } from "lucide-react"
...
function Chat() {
  ...
  const handleSend = async (e) => {
    ...
    try {
      ...
      setError(err.response || "An error occurred. Please try again.")
      console.error(err)
    } finally {
      setIsLoading(false)
    }
      ...
      <div className="flex flex-col h-screen bg-gradient-to-br from-purple-50 to-purple-100">
            ...
            <header className="flex items-center justify-between p-2 border-b border-purple-200">
              <div className="flex items-center space-x-2">
                <div className="p-2">
                  <img src="/assets/clork-logo.png" alt="Clork" className="w-8 h-8" />
                </div>
                <span className="font-semibold text-gray-800">Clork</span>
              </div>
              <button onClick={logout} className="p-2 rounded-full hover:bg-purple-100 text-gray-700">
              ...
              {messages.length === 0 ? (
                <div className="w-[1220px] mx-auto h-[480px] flex flex-col items-center justify-center text-center">
                  <div className="p-4 mb-4">
                    <img src="/assets/clork-logo.png" alt="Clork" className="w-12 h-12" />
                  </div>
                  <h2 className="text-2xl font-semibold text-gray-800 mb-2">Hi, {localStorage.getItem('username') || "there"}</h2>
                  <h3 className="text-xl font-semibold text-gray-700 mb-4">Can I help you with anything?</h3>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-4 w-full max-w-3xl">
                    {suggestions.map((suggestion) => (
                      ...
                      >
                        <div className="bg-purple-100 rounded-full p-2 inline-block mb-2">{suggestion.icon}</div>
                      ...
                      {message.user ? (
                        <div className="flex justify-end mb-2">
                          <div className="bg-purple-500 text-white p-3 rounded-lg rounded-tr-none max-w-[80%]">
                            <p>{message.user}</p>
                            <div className="text-xs text-purple-100 mt-1 text-right">{formatTimestamp(message.timestamp)}</div>
                          </div>
                        </div>
                      ) : (
                        <div className="flex mb-2">
                          <div className="bg-white text-gray-800 p-3 rounded-lg rounded-tl-none max-w-[80%] shadow-sm relative group border border-purple-200">
                            <p className="whitespace-pre-wrap">{message.chat}</p>
                            <div className="absolute top-2 right-2 opacity-0 group-hover:opacity-100 transition-opacity">
                              <button
                                onClick={() => copyToClipboard(message.chat, message.id)}
            ...
            <div className="w-[1240px] mx-auto rounded-lg mb-6 shadow-lg p-4 border-t border-purple-200 bg-white">
              <form onSubmit={handleSend} className="flex items-center space-x-2">
                <button type="button" className="p-2 rounded-full hover:bg-purple-100 text-gray-600">
                ...
                <button
                  ...
                  className={`p-2 rounded-lg ${
                    input.trim() ? "bg-purple-500 hover:bg-purple-600" : "bg-gray-300"
            ...
            <p className='text-sm text-gray-600 text-center mb-2'>Deepseek R1 Model Powered</p>
          </div>
  );
}
